...
public class CommonUtil {
    ...
    public static boolean isFilebasedTestcaseFolder(final File file) {
        if (file.isDirectory()) {
            ...
            if ((new File(GlobalSettings.getTOTestsCommonDir())).getName().toLowerCase().equals(file.getName().toLowerCase())) {
                return false;
            }
            ...
            for (File child : file.listFiles()) {
                if (child.isFile()) {
                    hasConfigProperties |= GlobalSettings.getTestcasePropertiesFileName().toLowerCase().equals(child.getName());
