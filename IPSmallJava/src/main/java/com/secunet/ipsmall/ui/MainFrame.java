...
public class MainFrame extends JFrame implements PropertyChangeListener {
    ...
    private void initialize() {
        ...
        mILogFolder.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                try {
                    if (Logger.getTestCasefile() != null) {
                       File l = new File(Logger.getTestCasefile());
                       Desktop.getDesktop().open(l.getParentFile());
                    }
                } catch (Exception exc)
                {
                    exc.printStackTrace();
    ...
    /**
     * Display the given test data in the main ui elements and enables/disables certain ui elements.
     * 
     * @param data
     *            the test data. must not be null
     */
    ...
    /**
     * Main method to start or cancel test case runs by checking the selected tree elements and filling/filtering the selections into the queue. than setting
     * the queue into the project and trigger the manager to start the test cases.
     */
    ...
    /**
     * Checks if testcase is running (ask if exit anyways) Checks if project was changed (ask if user wishes to save and exits when no error occured, does not
     * exit on cancel or close)
     */
    ...
    /**
     * updates the tree, queue and enables/disables certain interactive ui elements
     */
     ...
     * 
     * @param addition
     *            will be added after base title separated with space
     * @return title
     */
    ...
    /**
     * Loader to load huge log files in an own thread to increase responsiveness of UI.
     */
    private class LogWorker extends SwingWorker<String, String> {
        ...
        protected String doInBackground() throws Exception {
            try (FileReader fReader = new FileReader(file); BufferedReader reader = new BufferedReader(fReader)) {
                ...
                String result = transformXML(raw);
    ...
    /**
     * Must only be called in Event Dispatcher Thread! Adds a new line in the protocol containing the given text
     * 
     * @param stepDescr
     *            must not be null.
     */
