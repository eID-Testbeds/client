...
public class TestObjectSettings {
    ...
    public String getModule(String testcaseName) {
        File modulesDir = new File(testbedDir, GlobalSettings.getTestcaseTemplatesDir());
        return getModule(testcaseName, modulesDir);
    }
     ...
     *
     * @param testcaseName name of testcase.
     * @param modulesDir directory containing modules (with testcases).
     * @return Module name for testcase.
     */
    public String getModule(String testcaseName, File modulesDir) {
        String result = getModule(testcaseName, modulesDir, false);
        if (result.isEmpty()) {
            result = getModule(testcaseName, modulesDir, true);
        }
        return result;
    }
     ...
     * @return Module name for testcase.
     */
    private String getModule(String testcaseName, File modulesDir, boolean prefixSearch) {
        if (modulesDir.exists()) {
                ...
                if (module.isDirectory() && module.getName().startsWith("Module_")) {
                    ...
                    for (File testcase : testcases) {
                        if (testcase.isDirectory()) {
                            if (testcase.getName().equals(testcaseName) || (prefixSearch && testcase.getName().startsWith(testcaseName + "_"))) {
            ...
            Logger.TestObjectBuilder.logState("Unable to find DEFAULT path: " + modulesDir.getAbsolutePath(), IModuleLogger.LogLevel.Error);
        }
