...
public class ProfileSetup implements ITestObjectSetup {
        ...
        if (profile != null && !profile.isEnabled()) {
            if (profile.getTestcaseNames() != null) {
                ...
                for (String testcaseName : profile.getTestcaseNames()) {
                    String[] existingTestcases = getExistingTestcases(testcaseName);
                    
                    for (String testcase : existingTestcases) {
                        settings.updateTestcase(testcase, settings.getModule(testcase), TCCONFIG_LOAD, "false");
                    }
                    
                    if (existingTestcases.length == 0) {
                        Logger.TestObjectBuilder.logState("Unable to deactivate testcase " + testcaseName + ": no testcase configuration found.", IModuleLogger.LogLevel.Warn);
                    }
                }
    ...
    /**
     * Gets a list of existing test cases for a test case name.
     * @param testcaseName Name of the origin test case.
     * @return List of existing test cases.
     */
    private String[] getExistingTestcases(String testcaseName) {
        List<String> existingTestcases = new ArrayList<>();
        
        File moduleDir = new File(new File(settings.getTestObjectDir(), GlobalSettings.getTOTestsDir()), settings.getModule(testcaseName));
        for (File testcaseDir : moduleDir.listFiles()) {
            if (testcaseDir.getName().equals(testcaseName) || testcaseDir.getName().startsWith(testcaseName + "_")) {
                existingTestcases.add(testcaseDir.getName());
            }
        }
        
        return existingTestcases.toArray(new String[existingTestcases.size()]);
    }
