 ...
 * File Appender.
 */
@Plugin(name = "CustomXMLFileAppender", category = "Core", elementType = "appender", printObject = true)
public final class CustomXMLFileAppender extends AbstractOutputStreamAppender<CustomXMLFileManager> {
    ...
    private CustomXMLFileAppender(final String name, final Layout<? extends Serializable> layout, final Filter filter, final CustomXMLFileManager manager,
     ...
     */
    @PluginFactory
    public static CustomXMLFileAppender createAppender(
            // @formatter:off
        ...
        final CustomXMLFileManager manager = CustomXMLFileManager.getCustomXMLFileManager(fileName, isAppend, isLocking, isBuffered, advertiseUri,
            layout, bufferSize);
        if (manager == null) {
            return null;
        }

        return new CustomXMLFileAppender(name, layout, filter, manager, fileName, ignoreExceptions, isFlush,
